...
Bot は Anthropic の API キーを使いません。会話は Bot を動かすマシンの claude CLI をそのまま呼び出すため、**導入した人自身の Claude アカウントで動きます**。利用量もそのアカウントに紐づきます。

#### ⚠️ 会話履歴の保存先について（バックアップ対象に注意）

子どもとの**会話履歴の実体は、このリポジトリの中ではなく `~/.claude/projects/` 配下にあります**。Bot が保持しているのは、そこを指す claude セッションID（`data/conversation_sessions.json`）だけです。

これは claude CLI にセッション管理を任せている設計上の帰結で、次の点に注意してください。

- **`data/` だけをバックアップしても会話履歴は復元できません。** バックアップを取るなら `~/.claude/projects/` も対象に含めてください。
- **`~/.claude/projects/` を削除すると、それまでの会話の流れが失われます。** 残高や支出の記録（`data/` 配下の台帳）は消えませんが、AI が「これまで何を話してきたか」を思い出せなくなり、伴走の文脈が途切れます。
- Bot を動かすユーザーを変えると、参照先のホームディレクトリが変わるため過去の会話は引き継がれません。systemd で常駐させる場合は、実行ユーザーを固定してください。
- セッションIDには寿命（既定7日）があり、それを過ぎた発話では新しいセッションを張り直します。古すぎる文脈を引きずらないための仕様です。

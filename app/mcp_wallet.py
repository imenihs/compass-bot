...
def _tool_defs() -> list[dict]:
    ...
    base_defs = [
        ...
        {
            "name": "record_income",
            "description": "子どもの臨時入金を記録し、残高を増やす。自己申告のため1回の上限を超えると拒否する。**子がお金を受け取ったことを、済んだこととして話したら**、雑談で流さず必ず呼ぶこと。これからもらう予定の話では呼ばない（受け取ってから呼ぶ）。",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "name": {"type": "string", "description": "子どもの名前"},
        ...
        },
        {
            ...
            "description": (
                ...
                "迷ったら呼んでよい。合っていれば「ぴったり合ってたよ」と返るだけで、何も壊れない。"
            ),
            "inputSchema": {
                "type": "object",
                "properties": {

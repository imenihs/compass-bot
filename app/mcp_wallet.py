...
def _tool_defs() -> list[dict]:
    ...
    base_defs = [
        ...
        {
            "name": "record_income",
            "description": "子どもの臨時入金を記録し、残高を増やす。自己申告のため1回の上限を超えると拒否する。**子がお金を受け取ったことを、済んだこととして話したら**、雑談で流さず必ず呼ぶこと。これからもらう予定の話では呼ばない（受け取ってから呼ぶ）。**もらったのかどうか、金額がいくらか、あいまいなときは呼ばずに聞き返すこと。**",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "name": {"type": "string", "description": "子どもの名前"},
        ...
        },
        {
            ...
            "description": (
                ...
                "**自分の画面を見たいという意図が読み取れたら呼ぶ。**"
                "言い回しは決めない（ダッシュボード・マイページ・グラフ・URL、どう呼んでもよい）。"
        ...
        },
        {
            ...
            "description": (
                ...
                "判断の基準はひとつ。**すでに買った物について、"
                "その子が良し悪しを口にしたかどうか。**\n"
                "言い回しで判断しない。満足でも後悔でも、どちらも記録する価値がある。\n"
                "**残高は動かないので、迷ったら呼んでよい。**\n"
        ...
        },
        {
            ...
            "description": (
                "**財布チェック**。子が自分の手元にいくらあるかを金額で伝えてきたら呼ぶ。"
                "帳簿とズレていたら実際の金額に合わせる。\n"
                "判断の基準はひとつ。**金額を言ってきたのか、聞いてきたのか。**\n"
                "  ・言ってきた（本人が数えた・持っている額を伝えた）→ こちらを呼ぶ\n"
                "  ・聞いてきた（いくらあるか知りたい）→ get_balance を呼ぶ\n"
                "言い回しで判断しない。**その子が何をしようとしているかで判断する。**\n"
                "**迷ったら呼ばずに聞き返すこと。**「それって、いま持ってるお金のこと？」と確かめてから呼ぶ。"
                "帳簿を書き換える道具なので、あいまいなまま実行してはいけない。"
            ),
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        },
        {
            ...
            "description": (
                ...
                "**前のURLを使えなくしたい、という意図が明確なときだけ**呼ぶ。"
                "見たいだけのときは get_dashboard_url を使う。"
                "判断に迷ったら get_dashboard_url を選ぶこと"
                "（作り直すと前のURLが即使えなくなり、取り消せないため）。"
                "新しいURLはDMで届く。"
            ),
            "inputSchema": {
                "type": "object",
        ...
        },
        {
            ...
            "description": (
                ...
                "**子が目標のためにお金を積んだ、または立て替えを返したと話したときに呼ぶ。**言い回しではなく、そのお金が目標へ向かったかどうかで判断する。"
    ...
    if PARENT_MODE:
        parent_defs = [
            ...
            },
            {
                ...
                "description": (
                    "親が指定した子どもの残高を増やす/減らす調整をする。親が『たろうの残高を+500』『はなを-300』"
...
def _do_record_expense(args: dict) -> str:
    ...
    # 残高不足はここで止める（2026/08/09 追加）。冪等・二重適用は厳重に守っていたのに残高は無検査で、
    # 実機で 4700円 → -5300円 → -12300円 とマイナスが進行した。手持ちを超えた支出を黙って記録すると、
    # 「使ったつもり」が積み上がって子が自分の状況を把握できなくなり、家計簿として破綻する。
    # 立て替え払い（親の頼まれ物・部活の集金）は、そもそも先に親へ相談すべき局面であり、
    # 記録を通すのではなく、その相談へ誘導するのが正しい。
    if amount > before:
        return (
            f"今の残高は {before}円だから、{amount}円はまだ使えないんだ。記録もしていないよ。\n"
            f"あと {amount - before}円たりないね。\n"
            "もしもう払ったあとなら、立てかえになっているかもしれないから、おうちの人に相談してみてね。"
        )

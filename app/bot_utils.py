...
def _usage_guide_text() -> str:
    ...
    **決まった言い方（コマンド）は載せない**（2026/08/10）。
    以前は「残高おしえて」「貯金目標 ゲーム機 30000円」のような呪文を並べていたが、
    文字列一致の判定を全廃したため、この表は「効かない呪文の一覧」になっていた。
    いまは AI が言葉を汲むので、覚えることは無い。
    案内も「何ができるか」だけにして、言い方は本人に任せる。
    """
    return (
        "【Compass Bot のつかいかた】\n"
        "\n"
        "むずかしい言い方はいらないよ。ふつうに話しかけてね。\n"
        "\n"
        "■ こんなことができるよ\n"
        "・「300円でおかし買った」→ つかったお金を記録するよ\n"
        "・「お年玉 3000円もらった」→ もらったお金を記録するよ\n"
        "・「いまいくらある？」→ ざんだかをおしえるよ\n"
        "・「ゲーム機がほしい、30000円」→ ためる目標をつくるよ\n"
        "・「500円ためた」→ 目標にむけて積むよ（かえすお金も同じだよ）\n"
        "・「〇〇を買いたいんだけど」→ いっしょに考えるよ\n"
        "・「ダッシュボードが見たい」→ じぶんのページのURLをDMでおくるよ\n"
        "\n"
        "■ こまったとき\n"
        "うまく伝わらなかったら、言い方をかえてもう一度おしえてね。"
    )
...
def _usage_guide_text_parent() -> str:
    ...
    子ども向けと同じく、**コマンドは載せない**（2026/08/10）。
    「設定変更 たろう 固定 800円」のような形式は廃止し、
    金額の設定は Web ダッシュボードへ寄せた（桁の取り違えに気づけないため）。
    """
    return (
        "【Compass Bot（おうちの人むけ）】\n"
        "\n"
        "決まった言い方はありません。ふつうの日本語で話しかけてください。\n"
        "\n"
        "■ チャットでできること\n"
        "・「たろうに500円あげて」→ 支給します（誰に・いくらが曖昧なときは聞き返します）\n"
        "・「みんなの残高は？」「全体を見たい」→ 全員の状況を一覧で出します\n"
        "・「承認待ちある？」→ 子からの査定提案を出します\n"
        "・「たろうの設定は？」→ 固定額・臨時上限・フォロー方針の現在値を出します\n"
        "・「ダッシュボードを見たい」→ 管理画面のURLをDMで送ります\n"
        "・「安全設定を確認したい」→ 危険信号の通知先が正しいか実際に送って確かめます\n"
        "\n"
        "■ Web ダッシュボードでやること\n"
        "・固定お小遣い・臨時上限・AIフォロー方針の**変更**\n"
        "・立て替えの登録\n"
        "　金額の設定をチャットで受け付けないのは、桁を間違えても気づきにくいためです。\n"
        "\n"
        "■ 覚えておくと便利\n"
        "・URLを人に見られたら「URLを作り直して」と伝えてください（前のURLは使えなくなります）"
    )
